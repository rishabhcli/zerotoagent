  ...
  return (
    <div
      ...
      className={cn(
        ...
        glassSurfaceVariants({ variant: "table", interactive: false })
      )}
    >
      <table
        data-slot="table"
...
function TableFooter({ className, ...props }: React.ComponentProps<"tfoot">) {
  return (
    <tfoot
      ...
      className={cn(
        "border-t border-white/[0.08] bg-white/[0.06] font-medium [&>tr]:last:border-b-0",
        className
      )}
      {...props}
    />
...
function TableHead({ className, ...props }: React.ComponentProps<"th">) {
  return (
    <th
      ...
      className={cn(
        "h-12 px-4 text-left align-middle text-[0.72rem] font-medium whitespace-nowrap uppercase tracking-[0.18em] text-muted-foreground [&:has([role=checkbox])]:pr-0",
        className
      )}
      {...props}
    />
