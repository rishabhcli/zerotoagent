...
- **Voice (optional wow, never required)**: Users can speak an incident when away from a laptop; PatchPilot transcribes, confirms intent, then continues the workflow and posts a text receipt to chat + web.
...
> “PatchPilot, we’re seeing 500s in checkout after today’s deploy.  
...
> “PatchPilot: investigate this crash; reproduce and propose a tested fix.  
...
- User taps “Call PatchPilot” in the web UI.
...
PatchPilot must behave like a cautious, helpful teammate. Safety is not just “we asked for approval”; it is a clear, role-aware, user-friendly flow.
...
PatchPilot must show “what it is allowed to do” plainly:
...
- PatchPilot will still ingest evidence, propose a patch, and verify in sandbox
...
### What PatchPilot must capture every run (explicit checklist)
...
- A vertical timeline with expandable steps:
  ...
  - “What PatchPilot decided” (1–3 sentences)
  - “What PatchPilot did” (tool/sandbox receipts)
...
PatchPilot must “fail like a professional system” with next steps.

**Failure: Cannot reproduce the issue**

- What PatchPilot says:
...
## Advanced features that expand PatchPilot beyond thin-slice
...
**What it does:** When incidents span multiple repos (frontend + backend + shared SDK), PatchPilot can correlate symptoms and changes across repos, then propose a coordinated fix path.

**User experience:**

- User chooses “Multi-repo mode” in web UI (or says “this might be affecting frontend too” in chat).
- PatchPilot responds with:
...
**What it does:** PatchPilot distinguishes “real breakage” from flaky failures, reruns tests, and reports stability.
...
**What it does:** After local sandbox verification, PatchPilot can optionally trigger CI, wait for results, and attach CI proof to the receipts.
...
**What it does:** For severe incidents, PatchPilot can propose a rollback plan and gather evidence, but requires explicit approvals.
...
**What it does:** PatchPilot produces a transparent confidence score based on evidence, verification strength, and risk factors.
...
**What it does:** PatchPilot explains its decisions as a short story with evidence, not a wall of reasoning.
...
- PatchPilot produces a patch and verifies it in a sandbox with at least one concrete piece of proof:
  - tests passing, or
  - a deterministic repro harness passing after failing before
- PatchPilot requests explicit approval before opening a PR, and records who approved.
...
| Voice wow         | Optional voice summary and receipt      | Full voice workflow with live status                   | User taps “Call PatchPilot”                       | Voice response + text receipt + same Run Trace        |
...
1. **In chat:** “Here’s a real incident: 500s on checkout after deploy. I’m attaching a screenshot and logs. PatchPilot, fix it—only open a PR after tests pass and I approve.”
...
6. **PR proof:** Show the new PR with PatchPilot’s summary and verification receipts (tests passed + repro steps).
...
1. **End-to-end work, not commentary:** PatchPilot goes from messy incident evidence to a verified patch and PR—only after tests pass and a human approves.
